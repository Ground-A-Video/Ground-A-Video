...
class DDIMSampler(object):
    def __init__(self, diffusion, model, controlnet, schedule="linear", alpha_generator_func=None, set_alpha_scale=None, dtype=None):
        super().__init__()
    ...
    @torch.no_grad()
    def sample(self, S, shape, input, uc=None, uncond_embeddings=None, guidance_scale=1, mask=None, x0=None):
        self.make_schedule(ddim_num_steps=S)
        return self.ddim_sampling(shape, input, uc, uncond_embeddings, guidance_scale, mask=mask, x0=x0)
 

    @torch.no_grad()
    def ddim_sampling(self, shape, input, uc, uncond_embeddings, guidance_scale=1, mask=None, x0=None):
        b = shape[0]
        ...
        if uncond_embeddings is not None:
            uncond_embeddings_all_timesteps = uncond_embeddings.copy()
        else:
            uncond_embeddings_all_timesteps = None

        for i, step in enumerate(iterator):
            # select uc for timestep t
            if uncond_embeddings is not None and uncond_embeddings_all_timesteps is not None and i < len(uncond_embeddings_all_timesteps):
                uncond_embeddings = uncond_embeddings_all_timesteps[i]
            else:
            ...
            if mask is not None:
                ...
                do_inpaint = True  # inpainting [on / off]
                if do_inpaint:
                    img_orig = self.diffusion.q_sample( x0, input["timesteps"] )
                    img = img_orig * mask + (1. - mask) * img
                input["x"] = img.type(self.dtype) if img.type() != self.dtype else img
            
            img, pred_x0 = self.p_sample_ddim(input, index=index, uc=uc, uncond_embeddings=uncond_embeddings, guidance_scale=guidance_scale)
            if img.type() != self.dtype:
    ...
    @torch.no_grad()
    def p_sample_ddim(self, input, index, uc=None, uncond_embeddings=None, guidance_scale=1):
        
        down_block_res_samples, mid_block_res_sample = self.controlnet(
            sample=input["x"],
            timestep=input["timesteps"],
            encoder_hidden_states=input["context"],
            controlnet_cond=input["controlnet_cond"],
            conditioning_scale=input["controlnet_conditioning_scale"],
            return_dict=False,
        )
        ...
        if uc is not None and guidance_scale != 1:
            unconditional_input = dict(
                ...
                context=uncond_embeddings if uncond_embeddings is not None else uc,
            ...
            down_block_res_samples_uncond, mid_block_res_sample_uncond = self.controlnet(
                sample=unconditional_input["x"],
                timestep=unconditional_input["timesteps"],
                encoder_hidden_states=uncond_embeddings if uncond_embeddings is not None else uc,
                controlnet_cond=unconditional_input["controlnet_cond"],
                conditioning_scale=unconditional_input["controlnet_conditioning_scale"],
                return_dict=False,
            )
            down_block_res_samples_uncond = tuple(down_block_res_samples_uncond)
