...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, cross_attention_kwargs=None):
        for layer in self:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        # check dims
        assert dims==3, 'Conv2d -> PseudoConv3d if is_video'
...
class UNetModel3D(nn.Module):
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(SpatialTransformer3D(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=dim_head, depth=transformer_depth, fuser_type=fuser_type, use_checkpoint=use_checkpoint))
        ...
        self.middle_block = TimestepEmbedSequential(
            ResBlock(ch,
                     ...
                     use_checkpoint=use_checkpoint,
                     use_scale_shift_norm=use_scale_shift_norm),
            SpatialTransformer3D(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=dim_head, depth=transformer_depth, fuser_type=fuser_type, use_checkpoint=use_checkpoint),
            ResBlock(ch,
                     time_embed_dim,
                     dropout,
                     dims=dims,
    ...
    def restore_first_conv_from_SD(self):
        if self.first_conv_restorable:
            ...
            self.input_blocks[0][0] = conv_nd(3, 4, 320, 3, padding=1)
            # for compatibility
            from copy import deepcopy
            SD_weights["conv_temporal.weight"] = deepcopy(self.GLIGEN_first_conv_state_dict["conv_temporal.weight"])
            SD_weights["conv_temporal.bias"] = deepcopy(self.GLIGEN_first_conv_state_dict["conv_temporal.bias"])
            self.input_blocks[0][0].load_state_dict(SD_weights)
    ...
    def forward(self, input, down_block_additional_residuals=None, mid_block_additional_residual=None, cross_attention_kwargs=None):
        ...
        t_emb = t_emb.type(dtype=th.float16)    # assert float16
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        # Merge with ControlNet down_block outputs if exist
        if down_block_additional_residuals is not None:
            new_down_block_res_samples = ()
            for down_block_res_sample, down_block_additional_residual in zip(
                hs, down_block_additional_residuals
            ):
                down_block_res_sample = down_block_res_sample + down_block_additional_residual
                new_down_block_res_samples += (down_block_res_sample,)

            hs = list(new_down_block_res_samples)

        h = self.middle_block(h, emb, context, objs)

        # Merge with ControlNet mid_block outputs if exist
